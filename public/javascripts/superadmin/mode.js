...
function schedule_refresh() {
  socket.emit('client2server_mode', '');
...
function add_mode() {
  ...
  if ((slot1 != slot2) && (slot1 != slot3) && (slot1 != slot4) && 
    ...
    socket.emit('client2server_add_mode', name, mode, 'all');
  } else {
...
function deleteMode(e) {
  var mode = e.value;
  var row  = $table.bootstrapTable('getRowByUniqueId', mode);
  var site = row["site"];
  ...
  if (confirmation == mode) {
    socket.emit('client2server_delete_mode', mode, site);
  } else {
