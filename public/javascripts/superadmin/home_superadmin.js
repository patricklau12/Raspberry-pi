...
$(function() {
  ...
  socket.emit('client2server_monitor_refresh', '');
});
...
function set_dimming_floor(element) {
  var EPID = element.id;

  var dimming_id = '#' + EPID + 'dimming_floor';
  var DIMMING    = $(dimming_id).val();
...
function tick(id) {
  ...
  var msg = '<div class="btn-group" role="group">'
        ...
        + '  <button class="btn btn-link button-yellow" id="' + EPID + '" onclick="set_dimming(this)"  data-toggle="tooltip" data-placement="top" title="Set Dimming Level"><span class="fas fa-adjust"></span></button>'
...
socket.on('server2client_monitor_refresh', function(PropertyContent) {
  var property   = PropertyContent.split('=');
  var site_issue = '';

  if (property[10] == '1') {
    site_issue = '<span class="fas fa-exclamation"></span>';
  } else {
    site_issue = '<span class="fas fa-check"></span>';
  }

  mg_display = property[4] + ' / ' + property[5];
  sg_display = property[6] + ' / ' + property[7];
  ep_display = property[8] + ' / ' + property[9];
  ...
  $monitor_table.bootstrapTable('insertRow', {
    ...
    row: {
      ...
      contact_person: property[2],
      contact_phone:  property[3],
      mg:             mg_display,
      sg:             sg_display,
      ep:             ep_display,
      issue:          site_issue
  ...
  var existing = [];
  $("#site_select option").each(function() { 
    existing.push($(this).val());
  });

  if (existing.indexOf(property[1]) == -1) {
    $("#site_select").append($('<option></option>').val(property[1]).html(property[1]));
    $("#map_site_select").append($('<option></option>').val(property[1]).html(property[1]));
  }
});
...
function site_refresh() {
  ...
  if (site.indexOf('SELECT') == -1) {
    socket.emit('client2server_SA_site_refresh', site);
  }
...
socket.on('server2client_SA_site_refresh', function(blockList) {
  var existing = [];
...
function map_site_refresh() {
  var site = $('#map_site_select').val();

  if (site.indexOf('SELECT') == -1) {
    socket.emit('client2server_SA_map_site_refresh', site);
  }

  var i = 0;
  var L = document.getElementById("map_block_select").options.length - 1;
  
  for(i = L; i >= 1; i--) {
    document.getElementById("map_block_select").remove(i);
  }
...
socket.on('server2client_SA_map_site_refresh', function(blockList) {
  var existing = [];
  $("#map_block_select option").each(function() { 
    existing.push($(this).val());
  });
  ...
  for(i = 0; i < blockList.length; i++) {
    ...
    if (existing.indexOf(block[0]) == -1) {
      $("#map_block_select").append($('<option></option>').val(block[1]).html(block[0]));
    }
  }
});
...
socket.on('server2client_EP_floor', function(EPID, SGID, Connection, Light, functional_test, durational_test, blink_test, nickname, group, type, floor, watt) {
  ...
  var tick3 = '<div class="btn-group" role="group">'
            + '  <button class="btn btn-warning" id="' + EPID + '" onclick="set_dimming_floor(this)"  data-toggle="tooltip" data-placement="top" title="Set Dimming Level"><span class="fas fa-adjust"></span></button>'
            + '  <select class="form-control" id="' + EPID + 'dimming_floor">'
            + '    <option value="00" selected>0%</option>'
            + '    <option value="10">10%</option>'
            + '    <option value="20">20%</option>'
            + '    <option value="30">30%</option>'
